...
/**
 * A arte do cartao vem da CATEGORIA, nao de um array de fotos de barbearia.
 *
 * Numa barbearia sao as mesmas tres fotos de antes; numa manicure e a composicao em
 * violeta com o icone do oficio. `SegmentArt` decide, e esta tela nao precisa saber.
 */
function ProfessionalCard({ professional, serviceCount, position, businessType, onPress }: { readonly professional: Professional; readonly serviceCount: number; readonly position: number; readonly businessType: BusinessType; readonly onPress: () => void }) { return <Pressable onPress={onPress} style={({ pressed }) => [styles.professionalCard, pressed && styles.pressed]}><SegmentArt businessType={businessType} position={position} caption={professional.name} subtitle={`${serviceCount} serviço(s) disponível(is)`} /></Pressable>; }
...
const styles = StyleSheet.create({
  ...
  professionalGrid: { gap: 12 }, professionalCard: { overflow: "hidden", borderRadius: radius.modal, height: 190, backgroundColor: colors.surfaceRaised },
